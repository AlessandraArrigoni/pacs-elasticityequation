...
int main(int argc, char *argv[]) {
    ...
    const std::string data_file_name = command_line.follow("inputData/linear", 2, "-f",    "--file");

    GetPot dataFile(data_file_name.data());
    std::cout<< "File name : "<< data_file_name << std::endl;
   
 std::string intMethod(dataFile(std::string("bulkData/femspaces/integrationMethod" ).data (), "ciao") );
std::cout<<"metodo integrazione è:"<< intMethod<<std::endl;
